...
CdtrAgt_Ifsc=['EBNK0000002','FBNK0000002','GBNK0000002','HBNK0000002']
csv_file= "Messages_Debits.csv"
message_table_name="Message_Pacs8_Outward"
Transaction_Pacs8_outwards="Transaction_Pacs8_Outwards"
...
sql=f"insert into {Transaction_Pacs8_outwards}(TxId,BizMsgIdr,EndToEndId,IntrBkSttlmAmt,IntrBkSttlmDt,DbtrAgt,CdtrAgt) values (?,?,?,?,?,?,?)"
...
def generatedbtragent(messages):
    for message in messages:
        ...
        for i in range(10):
            ...
            DbtrAgt = message[1].replace('1', choice(branch_choice))
            CdtrAgt = choice(CdtrAgt_Ifsc)
...
def deletetransactionentries(con):
    curr = con.cursor()
    curr.execute(f"delete from {Transaction_Pacs8_outwards}")
